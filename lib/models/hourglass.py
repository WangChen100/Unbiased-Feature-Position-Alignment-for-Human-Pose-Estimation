...
class Hourglass(nn.Module):
    def __init__(self, block, num_blocks, planes, depth):
        ...
        # self.last_upsample = nn.Upsample(size=[4,3], mode='bilinear', align_corners=True)
        self.hg = self._make_hour_glass(block, num_blocks, planes, depth)
        self.cbr= self._make_cbr(planes, depth)
        self.relu=nn.ReLU(inplace=True)
        self.upsample = self._make_upsample(depth)
    ...
    def _cbr(self, planes):
        return nn.Sequential(nn.Conv2d(3, 2*planes, kernel_size=3, stride=2, padding=1, bias=False),
                             nn.BatchNorm2d(2*planes, momentum=BN_MOMENTUM),)

    def _make_cbr(self, planes, depth):
        cbrs=[]
        for _ in range(depth):
            cbrs.append(self._cbr(planes))
        return nn.ModuleList(cbrs)

    def _make_upsample(self, depth):
        sizes=[[5,4],[9,7],[17,13],[33,25],]
        up=[]
        for i in range(depth):
            up.append(nn.Upsample(size=sizes[i], mode='bilinear', align_corners=True))
        return nn.ModuleList(up)

    def _hour_glass_forward(self, n, x):
        up1 = self.hg[n-1][0](x)
        low1 = self.cbr[n-1](x)  # F.max_pool2d(x, 2, stride=2)
        low1 = self.relu(low1)
...
class HourglassNet(nn.Module):
    ...
    def __init__(self, block, cfg, **kwargs):
        ...
        for i in range(num_stacks):
            ...
            hg.append(Hourglass(block, num_blocks, self.num_feats, 4))  # 嵌套的hourglass结构
