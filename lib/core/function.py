...
def validate(config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_offset, target_weight, meta) in enumerate(val_loader):
            ...
            outputs = model(input)
            if isinstance(outputs, list):
                output = outputs[-1][:,:config.MODEL.NUM_JOINTS]
                output_offset = outputs[-1][:,config.MODEL.NUM_JOINTS:]
            else:
                output = outputs[:,:config.MODEL.NUM_JOINTS]
                output_offset = outputs[:,config.MODEL.NUM_JOINTS:]
            ...
            if config.TEST.FLIP_TEST:
                ...
                if isinstance(outputs_flipped, list):
                    ...
                    output_flipped = outputs_flipped[:,:config.MODEL.NUM_JOINTS]
                    offset_flipped = outputs_flipped[:,config.MODEL.NUM_JOINTS:]
            ...
            preds, maxvals = get_final_preds_offset(output.clone().cpu().numpy(),
                output_offset.clone().cpu().numpy(), 
                config.DATASET.LOCREF_STDEV, c, s, out_size)
